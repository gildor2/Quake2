...
/* There are three ways a function can be defined:
   (1) extern int foo() --- used for building and linking to the static library
	...
	ALL_STATIC is used as a synonym for ZLIB_STATIC (this helps when building
	packages with many dependencies).  However, ALL_STATIC overrides
	everything (since we cannot #undef something outside our namespace)
	Thus, precedence is: ALL_STATIC -> ZLIB_DLL (build) -> ZLIB_STATIC -> 
	(nothing) (link to dll).
...
#  endif
#  if defined (ZLIB_DLL)
#    ifdef ZLIB_STATIC   /* if ALL_STATIC, then ZLIB_DLL is undef'ed; won't get here */
#      undef ZLIB_STATIC
#    endif
#  endif
#  if defined (ZLIB_DLL)
