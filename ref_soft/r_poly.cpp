...
** R_PolygonDrawSpans
*/
// PGM - iswater was bool. changed to allow passing more flags
		...
		// adjust u to ceil the integer portion
			...
			for (v=itop ; v<ibottom ; v++)
			{
				if (u > 0)
					...
					pspan->u = 0;
				pspan->v = v;
#define ASSIGN(x)	*((unsigned*)&x)
				ASSIGN(pspan->pnext) = (unsigned)pspan->pnext & 0xF000FFFF | (v << 16);//!!
				u += u_step;
				pspan++;
			}
		}
	...
	ASSIGN(pspan->pnext) &= 0xFFFF; //!!
	ASSIGN(pspan->pnext) |= (0x8000 + lmaxindex) << 16; //!!
...
void R_PolygonScanRightEdge (void)
{
	...
	do
	{
		...
		// adjust u to ceil the integer portion
			...
			for (v=itop ; v<ibottom ; v++)
			{
				...
				pspan->count = ct;
				ASSIGN(pspan->pnext) = (unsigned)pspan->pnext & 0xFFFFF000 | v; //!!
				u += u_step;
				if (pspan->v != v) //!!!
				{
					pspan += v - pspan->v + 1;
	...
	ASSIGN(pspan->pnext) &= 0xFFFF0000; //!!
	ASSIGN(pspan->pnext) |= 0x8000 + s_maxindex; //!!
...
** R_ClipAndDrawPoly
*/
// PGM - isturbulent was bool. changed to int to allow passing more flags
void R_ClipAndDrawPoly ( float alpha, int isturbulent, bool textured )
{
