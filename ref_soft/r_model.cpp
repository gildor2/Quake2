...
void Mod_LoadVertexes (dvertex_t *data, int size)
{
	...
	out = (mvertex_t*)Hunk_Alloc ((size+8)*sizeof(*out));	// extra for skybox (else we can do loadmodel->vertexes=data)
...
void Mod_LoadEdges (dedge_t *data, int size)
{
	...
	in = data;
	out = (medge_t*)Hunk_Alloc ((size+13)*sizeof(*out));	// extra for skybox
...
void Mod_LoadTexinfo (texinfo_t *data, int size)
{
	...
	in = data;
	out = (mtexinfo_t*)Hunk_Alloc ((size+6)*sizeof(*out));	// extra for skybox
...
void Mod_LoadFaces (dface_t *data, int size)
{
	...
	in = data;
	out = (msurface_t*)Hunk_Alloc ((size+6)*sizeof(*out));	// extra for skybox
...
void Mod_LoadSurfedges (int *data, int size)
{
	...
	out = (int*)Hunk_Alloc ((size+24)*sizeof(*out));	// extra for skybox (if not this, we can just set pointer: loadmodel->surfedges=data!)
...
void Mod_LoadPlanes (dplane_t *data, int size)
{
	...
	in = data;
	out = (cplane_t*)Hunk_Alloc ((size+6)*sizeof(*out));		// extra for skybox
...
//
	...
	{
		mod->skins[i] = R_FindImage ((char *)pheader + pheader->ofsSkins + i*MD2_MAX_SKINNAME, it_skin);
	}
}
