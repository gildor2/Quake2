...
mleaf_t *Mod_PointInLeaf (vec3_t p, model_t *model)
{
	...
	if (!model || !model->nodes)
		Com_DropError ("GL_PointInLeaf: bad model");
...
==================
*/
static model_t *Mod_ForName (const char *name, bool crash)
{
	model_t	*mod;
	unsigned *buf;
	int		i;
...
//			break;
		default:	// should not happen! (map MUST be loaded by client)
			Com_DropError ("Internal error while loading BSP file");
		}
...
void Mod_LoadSubmodels (cmodel_t *data, int size)
{
	...
	in = data;
	out = (mmodel_t*)Hunk_Alloc (size*sizeof(*out));
...
void Mod_LoadEdges (dedge_t *data, int size)
{
	...
	in = data;
	out = (medge_t*)Hunk_Alloc ((size+1)*sizeof(*out));
...
void Mod_LoadTexinfo (texinfo_t *data, int size)
{
	...
	in = data;
	out = (mtexinfo_t*)Hunk_Alloc (size*sizeof(*out));
...
//			Com_DropError ("Bad surface extents");
	}
}
...
void Mod_LoadFaces (dface_t *data, int size)
{
	...
	in = data;
	out = (msurface_t*)Hunk_Alloc (size*sizeof(*out));
...
void Mod_LoadNodes (dnode_t *data, int size)
{
	...
	in = data;
	out = (mnode_t*)Hunk_Alloc (size*sizeof(*out));
...
void Mod_LoadLeafs (dleaf_t *data, int size)
{
	...
	in = data;
	out = (mleaf_t*)Hunk_Alloc (size*sizeof(*out));
...
//		Com_DropError ("MOD_LoadBmodel: bad surfedges count in %s: %i",
//		loadmodel->name, size);
...
void Mod_LoadPlanes (dplane_t *data, int size)
{
	...
	in = data;
	out = (cplane_t*)Hunk_Alloc (size*sizeof(*out));	//?? old:  "size*2*sizeof(*out)"; WHY ??!! (Q3 has the same ...); removed 13.03.2002
...
//--	in = (void *)(mod_base + l->fileofs);
	if (l->filelen % sizeof(*in))
		Com_DropError ("MOD_LoadBmodel: funny lump size in %s",loadmodel->name);
...
//--	in = (void *)(mod_base + l->fileofs);
	if (l->filelen % sizeof(*in))
		Com_DropError ("MOD_LoadHLBmodel: funny lump size in %s",loadmodel->name);
	count = l->filelen / sizeof(*in);
	out = Hunk_Alloc ( count*sizeof(*out));
...
//	if (i != HL_BSPVERSION)
//		Com_DropError ("Mod_LoadBrushModel: %s has wrong version number (%i should be %i)", mod->name, i, BSPVERSION);
...
void Mod_LoadAliasModel (model_t *mod, void *buffer)
{
	...
	pheader = (dmdl_t*)Hunk_Alloc (LittleLong(pinmodel->ofsEnd));
...
void Mod_LoadSpriteModel (model_t *mod, void *buffer)
{
	...
	if (sprout->version != SP2_VERSION)
		Com_DropError ("%s has wrong version number (%i should be %i)", mod->name, sprout->version, SP2_VERSION);
