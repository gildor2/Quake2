...
void GL_LerpVerts (int nverts, dTriVertx_t *v, dTriVertx_t *ov, dTriVertx_t *verts,
...
void GL_DrawAliasFrameLerp (dmdl_t *paliashdr, float backlerp)
{
	...
	frame = (dAliasFrame_t *)((byte *)paliashdr + paliashdr->ofsFrames
		+ currententity->frame * paliashdr->frameSize);
	verts = v = frame->verts;

	oldframe = (dAliasFrame_t *)((byte *)paliashdr + paliashdr->ofsFrames
		+ currententity->oldframe * paliashdr->frameSize);
	ov = oldframe->verts;

	order = (int *)((byte *)paliashdr + paliashdr->ofsGlcmds);
	...
	GL_LerpVerts (paliashdr->numXyz, v, ov, verts, lerp, move, frontv, backv);
	...
	if ( gl_vertex_arrays->integer )
	{
...
#else
				colorArray[i*3+0] = shadelight[0];
				colorArray[i*3+1] = shadelight[1];
				colorArray[i*3+2] = shadelight[2];
#endif
			}
		}
		...
		while (1)
		{
			...
			if ( currententity->flags & (RF_SHELL_RED|RF_SHELL_GREEN|RF_SHELL_BLUE|RF_SHELL_DOUBLE|RF_SHELL_HALF_DAM) )
			{
				do
				{
					// order[0] = float s, order[1] = float t, order[2] = vertexIndex
					index_xyz = order[2];
					order += 3;
...
static qboolean R_CullAliasModel( vec3_t bbox[8], entity_t *e )
{
	...
	pframe = (dAliasFrame_t *) ((byte *) paliashdr + paliashdr->ofsFrames + e->frame * paliashdr->frameSize);

	poldframe = (dAliasFrame_t *) ((byte *) paliashdr + paliashdr->ofsFrames + e->oldframe * paliashdr->frameSize);
...
void R_DrawAliasModel (entity_t *e)
{
	...
	else
	{
		...
		// big hack!
		// (used by game for monster sight AI)
