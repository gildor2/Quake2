...
public:
	...
	int			fogNum;				// != 0 when surface inside volumetric fog brush
...
public:
	...
	CLight		*light;
	planeDlight_t *dlights;			// dynamic lighting info
...
public:
	...
	int			*indexes;
	trisurfDlight_t *dlights;		// dynamic lighting info
	virtual void Tesselate(refEntity_t &ent);
	virtual void AddToSky(bool showDebug);
};
...
{
	bool		hasSurface;
	CPlane		surface;		// valid when hasSurface==true only
	// info from brush
	CBox		bounds;
	// info from shader
	color_t		color;
