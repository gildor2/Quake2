...
static shader_t *FinishShader()
{
	...
	for (numStages = 0; numStages < MAX_SHADER_STAGES; numStages++)
	{
		...
		// replace lightmap blend src*dst -> src*dst*2
		if (numStages == 1 && st[0].isLightmap ||		// lightmap is on the 1st stage
			numStages > 0 && st[numStages].isLightmap)	// previous stages - texturing, current - lightmap
		{
...
shader_t *FindShader(const char *name, unsigned style)
{
	...
	for (shader_t *shader = hashTable[hash]; shader; shader = shader->hashNext)
		if ((shader->Name == Name2) &&
			(shader->style == (style & SHADER_STYLEMASK) || shader->scripted))
		{
			if (shader->lightmapNumber == lightmapNumber)
				return shader;				// exact as required

			if (shader->lightmapNumber >= 0 && lightmapNumber == LIGHTMAP_RESERVE)
