...
//?? replace with global true/false
#define TRUE	1
#define FALSE	0
...
int Zip_ExtractFileMem (FILE *F, zip_file *file, void *buf)
{
	...
	if (file->crc32 && file->crc32 != crc32 (0, (uch*)buf, file->ucsize)) return FALSE;	// bad CRC or size
...
ZFILE *Zip_OpenFile (FILE *F, zip_file *file)
{
	...
	{   // inflate file
		...
		z->s.avail_in = 0;
		z->buf = (char*)malloc (READ_BUF_SIZE);
		if (!z->buf)
		{
			free (z);
...
//?? replace all malloc()/free() with local memory manager
